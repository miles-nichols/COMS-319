...
function read_matrix(matrix) {
  ...
  for (let i = 1; i < matrix.length; i++) {
    ...
    let sepalLength = parseFloat(row[0]);
    let sepalWidth = parseFloat(row[1]);
    let petalLength = parseFloat(row[2]);
    let petalWidth = parseFloat(row[3]);
    let species = row[4];

    // Update sums and counters for each species
    if (species === "Iris-setosa") {
      setosa_sepal_Length += sepalLength;
      setosa_sepal_Width += sepalWidth;
      setosa_petal_Length += petalLength;
      setosa_petal_Width += petalWidth;
      setosa_count++;
    } else if (species === "Iris-versicolor") {
      versicolor_sepal_Length += sepalLength;
      versicolor_sepal_Width += sepalWidth;
      versicolor_petal_Length += petalLength;
      versicolor_petal_Width += petalWidth;
      versicolor_count++;
    } else if (species === "Iris-virginica") {
      virginica_sepal_Length += sepalLength;
      virginica_sepal_Width += sepalWidth;
      virginica_petal_Length += petalLength;
      virginica_petal_Width += petalWidth;
      virginica_count++;
    }
  }
}
...
function compute_averages() {
  if (setosa_count > 0) {
    setosa_sepal_Length /= setosa_count;
    setosa_sepal_Width /= setosa_count;
    setosa_petal_Length /= setosa_count;
    setosa_petal_Width /= setosa_count;
  }

  if (versicolor_count > 0) {
    versicolor_sepal_Length /= versicolor_count;
    versicolor_sepal_Width /= versicolor_count;
    versicolor_petal_Length /= versicolor_count;
    versicolor_petal_Width /= versicolor_count;
  }

  if (virginica_count > 0) {
    virginica_sepal_Length /= virginica_count;
    virginica_sepal_Width /= virginica_count;
    virginica_petal_Length /= virginica_count;
    virginica_petal_Width /= virginica_count;
  }
}
